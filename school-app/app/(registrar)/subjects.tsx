...
export default function RegistrarSubjects() {
  ...
  const fetchSubjects = useCallback(async () => {
    try {
      const res = await api.get('/subjects', { params: { search: search.trim() || undefined } });
  ...
  useEffect(() => { fetchSubjects(); fetchCourses(); }, [fetchSubjects, fetchCourses]);

  const openAdd = () => { setEditing(null); setForm(EMPTY_FORM); setCourseDropdownOpen(false); setShowModal(true); };
  const openEdit = (sub) => {
    setEditing(sub);
    setForm({
        ...
        <ScrollView contentContainerStyle={s.list}
          ...
          {subjects.length === 0 ? (
            <View style={s.emptyWrap}>
              ...
              <Text style={s.emptySub}>{'Tap "+ Add" to create your first subject.'}</Text>
            </View>
          ) : subjects.map((sub, i) => (
      ...
      <Modal visible={showModal} animationType="slide" presentationStyle="pageSheet">
        <View style={s.modal}>
          ...
          <ScrollView contentContainerStyle={s.modalBody} keyboardShouldPersistTaps="handled">
            <Field label="Subject Code *"><Input value={form.code} onChangeText={v => set('code', v)} placeholder="e.g. IT101" /></Field>
            <Field label="Subject Name *"><Input value={form.name} onChangeText={v => set('name', v)} placeholder="e.g. Introduction to Programming" /></Field>
            ...
            <Field label="Program Type">
              <ChipGroup options={[{ key: 'college', label: '🎓 College' }, { key: 'shs', label: '🏫 SHS' }]}
                value={form.program_type} onSelect={v => { set('program_type', v); set('course', ''); set('strand', ''); }} />
            </Field>
            ...
            {form.scope === 'course' ? (
              <Field label="Course / Strand">
                {form.program_type === 'college' ? (
                  ...
                  <Input value={form.strand}
                    onChangeText={v => set('strand', v)}
            ...
            <TouchableOpacity style={[s.saveBtn, saving && { opacity: 0.6 }]} onPress={handleSave} disabled={saving}>
              {saving ? <ActivityIndicator color="#fff" /> : <Text style={s.saveBtnText}>{editing ? 'Update Subject' : 'Create Subject'}</Text>}
            </TouchableOpacity>
